 ...
 */
public class ParticularVenue implements MySQLRead
{
    private int sno;
    ...
    public ParticularVenue(String tableName)
    {
    ...
    public boolean deleteAll()
		...
		{
                    success  = true; 
                    while(venueList.getData() )
                            success = newMysql.deleteAll(venueList.getVenueName());
                        
		}
		return success;
           
        }
    public void updateVenueAllotmentArray(int index, int updateValue, int slotLength) 
    {
        if(!this.tableName.equals(""))
        {
            int jIndex = index%5;
    ...
    public boolean isFree(int index, int slotLength)
    {
        ...
        if(!this.tableName.equals(""))
        {
            if(slotLength == 9)
            {
                res = new ParticularVenueTimeTable(tableName+"_tt").isFree(index);
            }
            else
            {
                            int realIndex;;
...
/*public class ParticularVenue implements MySQLRead
{
	...
	public void setValue(String [] valueList)
	{
		for(int i=0;i<valueList.length;i++)
		   this.venueAllotmentArray[i]=Integer.parseInt(valueList[i]);
	}
...
*/
  /* public ParticularVenue(String tableName)7	{
	   this.tableName=tableName;
	   venueAllotmentArray = new int[500];
	...
	public void setData(int [] venueAllotmentArray)
	{
	   StringBuilder venueAllotmentArrayData = new StringBuilder();		
	   for(int i=0;i<venueAllotmentArray.length-1;i++)
	   {
		   this.venueAllotmentArray[i]=venueAllotmentArray[i];
		   venueAllotmentArrayData.append(venueAllotmentArray[i]+",");
	   }
       this.venueAllotmentArray[venueAllotmentArray.length-1]=venueAllotmentArray[venueAllotmentArray.length-1];
	   venueAllotmentArrayData.append(venueAllotmentArray[venueAllotmentArray.length-1]);
	   	   
		int res=mysql.insertData(getTableName(),venueAllotmentArrayData.toString());  
			if(res!=0)
				System.out.println("Row inserted successfully...");// delete when complete....
			else
				System.out.println("Row couldn't be inserted....");// delete when complete....
	...
	public boolean display()
	{
		...
		if(res)
		{	
		 System.out.println("Venue Name: "+getTableName()+"\n");
		 for (int i=0;i<100 ;i++ )
		 {
		 	for (int j=0;j<5;j++ ) 
		 	{
		 	 System.out.print(venueAllotmentArray[i*j+j]+"\t");		
		 	}	
		 	System.out.print("\n");
		 }
		} 
	...
	public int[] getVenueAllotmentArray()
	{
		return this.venueAllotmentArray;
	}
	public int getAvailability(int linearIndex)
	{
		if (linearIndex<500)
			return venueAllotmentArray[linearIndex];
		else
			return -1;
	}
	public int getAvailability(int slotIndex,int dayIndex )//(100X5)
	{
		if(slotIndex*dayIndex+dayIndex<500)
			return venueAllotmentArray[slotIndex*5+dayIndex];
		else
			return -1;
	}
	public boolean isExist()
    ...
    public void updateVenueAllotmentArray(int activeLabelIndex, int i, int slotLength) {
